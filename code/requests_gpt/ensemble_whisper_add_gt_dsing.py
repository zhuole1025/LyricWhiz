...
def load_gt(song_name):
    gt_dir="/home/jiahaop/workspace/LyricWhiz/datasets/dsing/lyrics"
    official = os.path.join(gt_dir, song_name + '.txt')
...
def deal_with_samples(t_text):
    ...
    t_text = t_text.translate(str.maketrans("", "", ",.!?"))
    t_text = t_text.strip()
    return t_text

def deal_with_ann(o_text):
    o_text = o_text.lower()
...
def ensemble_whisper(song_name, whisper_rlts_dir, sample_num, content_type, audio_type):
    ...
    whisper_rlts = [deal_with_samples(s) for s in whisper_rlts]
    whisper_rlts.append(deal_with_ann(gt))
    
    random.shuffle(whisper_rlts)
