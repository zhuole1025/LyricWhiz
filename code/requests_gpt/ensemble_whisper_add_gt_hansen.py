...
def load_gt(song_name):
    gt_dir="/home/jiahaop/workspace/LyricWhiz/datasets/hansen/annotations"
    official = os.path.join(gt_dir, song_name + '_sents.txt')
    if 'trick_me' in song_name or 'umbrella' in song_name:
        o_text = []
        with open(official, 'r') as _file:
            for line in _file:
                o_text.extend(line.split()[2:])
    else:
        o_text = np.loadtxt(official, dtype=str, delimiter='\t', usecols=2)
    return o_text
...
def ensemble_whisper(song_name, whisper_rlts_dir, sample_num, content_type, audio_type):
    ...
    gt = load_gt(song_name)
    whisper_rlts.append("\n".join(gt).lower().replace('a mulatto breath*', '').replace('a denial breath*', '').replace('breath*', ''))
...
if __name__ == "__main__":
    ...
    prompt_recorded_file = os.path.join(output_dir, "prompt_final.txt")
